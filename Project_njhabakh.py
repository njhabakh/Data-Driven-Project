...
for i in range(len(name)):
    print str(name[i])+":"+str((data['Time'][k[i]])-(data['Time'][indices[i]]))
...
data5=data[data['Point_name']==name[5]] #Data only for power meter 'Scaife Hall Electric (Shark 34) - Demand Watts '
...
plt.plot(data2['Time'],data2['Value'])
plt.title(name[2])
plt.xlabel('Timestamp')
plt.ylabel('Power in Watts')
fig3= plt.figure(figsize=(15,5))
plt.plot(data0['Time'],data0['Value'])
plt.title(name[0])
plt.xlabel('Timestamp')
plt.ylabel('Power in Watts')
...
time_temp=time_temp3=time_temp2=time_temp0=[temp[i][0] for i in range(len(temp))]
F_temp=F_temp3=F_temp2=F_temp0=[float(temp[i][1]) for i in range(len(temp))]
...
data_new=[{'Time'},{'Power'},{'Temperature'}]
...
p=[];
p.append(data['Value'][indices[0]:k[0]])
p
